...
function createAuthRouter(platform) {
  ...
  router.use((error, req, res, next) => {
    ...
    res.status(400).json({ error: error.message || "Auth error." });
  });
...
function createAuditRouter(platform) {
  ...
  router.post("/leads", async (req, res) => {
    try {
      ...
      res.json({ ok: true, created: result.created, leadId: result.lead?.id });
    } catch (error) {
      res.status(400).json({ error: error.message });
    }
  });
  ...
  router.get("/report/:id", async (req, res) => {
    ...
    try {
      ...
      if (!report) return res.status(404).json({ error: "Report not found." });
      res.json({ ...report, full_report_available: true });
    } catch (error) {
      res.status(400).json({ error: error.message });
    }
  });
  ...
  router.get("/report/:id/pdf", async (req, res) => {
    ...
    try {
      ...
      res.json({ pdf: pdfBuffer.toString("base64"), filename: `storecops-report-${report.report_id || report._id}.pdf` });
    } catch (error) {
      res.status(400).json({ error: error.message });
    }
  });
  ...
  router.post("/report/:id/email", async (req, res) => {
    ...
    try {
      ...
      res.json({ delivered: result.delivered, email, report_id: report._id });
    } catch (error) {
      res.status(400).json({ error: error.message });
    }
  });
